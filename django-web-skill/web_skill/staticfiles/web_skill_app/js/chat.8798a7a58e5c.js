...
async function sendMessage(message) {
    try {
        ...
        if (response.ok) {
            ...
            // Verificar si finalizó y hay dashboard_id
            if (data.finalizado && data.dashboard_id) {
                const dashboardUrl = `/encuesta/dashboard/?dashboard_id=${data.dashboard_id}`;
                setTimeout(() => {
                    showFinalizacionModal(dashboardUrl);
                }, 1000);
            }
        } else {
            // Mostrar error
